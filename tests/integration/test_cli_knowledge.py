...
def test_knowledge_list_and_show_json_output():
    ...
    assert list_payload[0]["id"] == "kn01", f"expect result to be {'kn01'}, got {list_payload[0]['id']}"
    assert show_payload["id"] == "kn01", f"expect result to be {'kn01'}, got {show_payload['id']}"
    assert show_payload["title"] == "Deploy Steps", f"expect result to be {'Deploy Steps'}, got {show_payload['title']}"
