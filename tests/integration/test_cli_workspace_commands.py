...
def test_workspace_list_and_show_json_output():
    ...
    assert list_payload[0]["id"] == ws.id, f"expect list_payload[0]['id'] == ws.id, got {list_payload[0]['id'] == ws.id}"
    assert show_payload["id"] == ws.id, f"expect show_payload['id'] == ws.id, got {show_payload['id'] == ws.id}"
    assert "network_summary" in show_payload, f"expect 'network_summary' in show_payload, got {'network_summary' in show_payload}"
